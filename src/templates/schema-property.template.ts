...
export function SchemaPropertyTemplate(prop: SchemaPropertyType, endpointChangeType: ChangeType, added: string[], deleted: string[]): string {
    ...
    const isCreate: boolean = added.includes(prop.name);
    const isDelete: boolean = deleted.includes(prop.name);
    ...
    return `
<tr class="property-row ${prop.required ? 'required':''}" ${useColor && !prop?.$ref ? `style="background-color: ${changeColor}` : ''}">
